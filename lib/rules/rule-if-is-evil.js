...
function checkDirective(directiveNode, errors, state) {
    ...
    if (state.directiveCount > 1) {
        errors.push('if is evil and must only contain a single directive');
    }
    if (directiveNode.name == 'rewrite') {
        state.inRewrite = true;
        state.rewriteParameterCount = 0;
    } else if (directiveNode.name !== 'return') {
        errors.push(`if is evil and must only contain 'rewrite' or 'return' directive, found '${directiveNode.name}'`);
    }
    return state;
}
    ...
    if (state.inRewrite) {
        ...
        if (state.rewriteParameterCount === 3) {
            if (parameterNode.text !== 'last') {
                errors.push(`if is evil and must only contain 'rewrite' directives using the 'last' flag, found '${parameterNode.text}'`);
            }
        }
    }
    return state;
